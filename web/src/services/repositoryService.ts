...
export class RepositoryService {
    ...
    ): Promise<{ success: boolean; repositories?: DatabaseRepository[]; error?: string }> {
        try {
     ...
     */
    static async getUserRepositories(userId: string): Promise<{ repositories: DatabaseRepository[]; error?: string }> {
        try {
            const { data, error } = await supabase
                .from('repositories')
                .select('*')
            ...
            return { repositories: [], error: error instanceof Error ? error.message : 'Unknown error' }
        }
    }
    ...
    static async getRepository(
        ...
        try {
            ...
            return { repository: undefined, error: error instanceof Error ? error.message : 'Unknown error' }
        }
    }
     ...
     * Delete a repository
     */
    static async deleteRepository(repositoryId: string): Promise<{ success: boolean; error?: string }> {
        try {
            const { error } = await supabase
                .from('repositories')
                .delete()
    ...
    static async syncRepositoriesFromGitHub(
        ...
        try {
            const repositoryData: RepositoryCreateData[] = githubRepositories.map(repo => ({
                user_id: userId,
                name: repo.name,
                owner: repo.owner,
