...
export default async function handler(
  ...
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      console.log(
        '⚠️  No Authorization header found, checking server-side session',
      )
    ...
    console.log(
      `📂 Found ${repositoriesResult.data.length} repositories (source: ${repositoriesResult.source})`,
    )
    ...
    console.log(
      `  - Limited repositories: ${commitsResult.data.limitedRepositories}`,
    )
