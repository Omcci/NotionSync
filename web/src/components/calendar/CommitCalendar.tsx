...
export function CommitCalendar({
        ...
        <div className="commit-list p-2 space-y-1">
          {displayCommits.map((commit: Commit, index: number) => {
            ...
            const truncatedMessage =
              message.length > 22 ? `${message.substring(0, 22)}...` : message

            return (
              <div
                key={index}
                className="commit-item text-xs text-gray-600 dark:text-gray-300"
              >
  ...
  const renderDay = (
    ...
    const isHovered =
      hoveredDate &&
      format(hoveredDate, 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd')
    ...
    let dayClasses =
      'calendar-day-cell relative transition-all duration-200 cursor-pointer border-r border-b last:border-r-0 min-h-[120px] p-3'
    ...
    if (isHovered) {
      dayClasses +=
        ' z-[9998] relative shadow-2xl ring-4 ring-blue-500/60 dark:ring-blue-400/60 bg-white dark:bg-gray-800 scale-[1.05] border-blue-300 dark:border-blue-600'
    }
    ...
    if (hasSummary && isCurrentMonth) {
      if (isToday) {
        ...
        dayClasses +=
          ' ring-2 ring-amber-400 dark:ring-amber-500 bg-gradient-to-br from-blue-50 via-amber-50 to-blue-50 dark:from-blue-900/20 dark:via-amber-900/20 dark:to-blue-900/20'
      } else {
        // Just has summary
        dayClasses +=
          ' bg-gradient-to-br from-amber-50 to-yellow-50 dark:from-amber-900/20 dark:to-yellow-900/20 border-amber-300 dark:border-amber-700 ring-1 ring-amber-200 dark:ring-amber-800'
      }
    }
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            {hasSummary && isCurrentMonth && (
              <div
                className="w-2 h-2 bg-amber-500 dark:bg-amber-400 rounded-full shadow-sm animate-pulse"
          ...
          <>
            ...
            <div
              ...
              style={{
                ...
                width: '40rem', // 640px
                transformOrigin: shouldShowOnLeft
                  ? 'right center'
                  : 'left center',
              }}
              ...
              <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl overflow-hidden cursor-pointer hover:shadow-3xl transition-shadow transform scale-100 hover:scale-[1.02] relative">
                ...
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    {dayCommits.length} commit
                    {dayCommits.length !== 1 ? 's' : ''} across {repoCount}{' '}
                    repositor{repoCount !== 1 ? 'ies' : 'y'} • Click for full
                    details
                  </p>
                </div>
                ...
                <div className="p-4 max-h-64 overflow-y-auto">
                  <div className="space-y-4">
                    {Object.entries(commitsByRepo).map(
                      ([repoName, repoCommits], repoIndex) => (
                          ...
                          <div className="space-y-2 ml-4">
                            {repoCommits.slice(0, 3).map((commit, index) => {
                              ...
                              return (
                                <div
                                  key={index}
                                  className="flex gap-3 p-2 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"
                                >
                            ...
                            {repoCommits.length > 3 && (
                              <div className="text-xs text-gray-500 dark:text-gray-400 text-center">
                                +{repoCommits.length - 3} more commits in{' '}
                                {repoName}
                              </div>
                            )}
                          </div>
