...
const RepositoryManager: React.FC = () => {
    ...
    const toggleSyncEnabled = (repoId: string) => {
        const updatedRepos = repos.map(repo =>
            repo.id === repoId
                ? { ...repo, syncEnabled: !repo.syncEnabled }
                : repo
        )
        setRepos(updatedRepos)
    }
                                ...
                                <div className="max-h-96 overflow-y-auto space-y-3">
                                    {filteredAvailableRepos.length === 0 ? (
                                        <div className="text-center py-8 text-gray-500">
                                            {searchTerm ? 'No repositories found matching your search' : 'All repositories are already added'}
                                        </div>
                                    ) : (
                                                ...
                                                <CardContent className="p-4">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex-1">
                                                            ...
                                                            <p className="text-sm text-gray-500">{repo.owner}/{repo.name}</p>
                                                            {repo.description && (
                    ...
                    <CardContent>
                        {repos.length === 0 ? (
                            <div className="text-center py-8 text-gray-500">
                                ...
                                <p className="text-sm">Add repositories to start syncing with Notion</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {repos.map((repo) => (
                                                    ...
                                                    <div className="flex items-center space-x-3">
                                                        ...
                                                        <div>
                                                            <div className="flex items-center space-x-2">
                                                                ...
                                                                <Badge
                                                                    variant={(repo.syncEnabled ?? true) ? "default" : "secondary"}
                                                                    className="text-xs"
                                                                >
                                                                    {(repo.syncEnabled ?? true) ? 'Syncing' : 'Paused'}
                                                                </Badge>
                                                            </div>
                                                            <p className="text-sm text-gray-500">{repo.owner}/{repo.name}</p>
                                                            {repo.lastSync && (
                                                    ...
                                                    <AlertDialog>
                                                        ...
                                                        <AlertDialogContent>
                                                            <AlertDialogHeader>
                                                                ...
                                                                <AlertDialogDescription>
                                                                    Are you sure you want to remove &quot;{repo.name}&quot; from sync?
                                                                    This will stop syncing commits from this repository to Notion.
                                                                </AlertDialogDescription>
                                                            </AlertDialogHeader>
                                                            <AlertDialogFooter>
