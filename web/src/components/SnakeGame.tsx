...
const COLORS = { GAME_OVER: '#D24D57', FRUIT: '#EC644B', HEAD: '#336E7B', BODY: '#C8F7C5', BOARD: '#86B5BD' }

const SnakeGame = () => {
  const [board, setBoard] = useState(Array(BOARD_SIZE).fill(Array(BOARD_SIZE).fill(false)))
        ...
        {board.map((row, rowIndex) =>
          row.map((col, colIndex) => (
            <div
              ...
              className="w-5 h-5"
              style={{
                backgroundColor: snake.some((part) => part.x === colIndex && part.y === rowIndex)
                  ? COLORS.HEAD
